...
DEFAULT_VOLUME_ROOT = Path("/data/dendritron-stage1")
PUNCTUATION_V2_ROOT = Path("/data/dendritron-stage1-punctuation-v2")
...
def configure_output_root(value: str | Path) -> Path:
    """Select one narrow Stage-1 directory on the mounted data volume.

    The completed pre-punctuation inventory remains preserved at
    ``/data/dendritron-stage1``.  A corrected recount can therefore target
    ``/data/dendritron-stage1-punctuation-v2`` and be compared before any
    donor Engram rows are replaced.
    """

    root = Path(value)
    if not root.is_absolute() or root.parent != Path("/data"):
        raise ValueError("Stage-1 output root must be a direct child of /data")
    if not root.name.startswith("dendritron-stage1"):
        raise ValueError("Stage-1 output directory must begin dendritron-stage1")
    if root in {Path("/data"), Path("/")}:
        raise ValueError("Stage-1 output root is too broad")

    global VOLUME_ROOT, WORK_DIR, PARTIAL_DIR, HASH_DIR, STATE_DIR, FINAL_DIR
    VOLUME_ROOT = root
    WORK_DIR = root / "work"
    PARTIAL_DIR = WORK_DIR / "partials"
    HASH_DIR = WORK_DIR / "document-hashes"
...
def build_corpus(
    force: bool = False,
    output_root: str = str(DEFAULT_VOLUME_ROOT),
) -> dict[str, Any]:
    configured_root = configure_output_root(output_root)
    if force:
    ...
    result = {
        "status": "complete",
        "volume": VOLUME_NAME,
        "stage1_root": str(configured_root),
...
def main(
    ...
    result = build_corpus.remote(force=force, output_root=output_root)
    print(json.dumps(result, indent=2))
