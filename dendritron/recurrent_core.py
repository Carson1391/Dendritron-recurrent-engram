...
from .working_adapter import SkillExpertStats, SkillExpertSystem
...
class TwoBlockRecurrentCore(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # N is the block-equivalent unrolled depth. Beta initializes designated
        # residual-branch matrices once and never multiplies a recurrent visit.
            ...
            nn.ModuleList(
                [
                    LNGramMemory(
                        ...
                        residual_initialization_gain=self.beta,
                    )
                    for _ in range(2)
                ]
            )
        ...
        self.skill_expert = SkillExpertSystem(
            config.model_width,
            universal_rank=config.universal_rank,
            universal_top_k=config.universal_top_k,
            universal_bootstrap_seed=config.universal_bootstrap_seed,
            skill_count=config.skill_count,
            skill_rank=config.skill_rank,
        ...
        for round_index in range(total_rounds):
            for block_index in range(2):
                ...
                contraction_residual = (
                    harmax_update + memory_update + lngram_update
                )
