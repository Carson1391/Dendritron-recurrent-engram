...
> **v1.3 Euclidean HarMax correction:** every HarMax contraction uses the
> ordinary Euclidean displacement and squared Euclidean distance. The learned
> \(L^\top L\) metric from v1.2 is revoked. DeepLoop \(\beta\) initializes the
> designated matrices in the LNGram, Hash-Engram, shared-skill, and expert
> residual branches once; \(\alpha\) remains the per-visit skip scale.
...
| ID | Decision | Status |
|---|---|---|
| D-001 | The first recipient uses the tokenizer from `Qwen/Qwen3.6-35B-A3B` at the exact resolved revision recorded by the completed Stage-2 donor manifest. It provides input IDs, token embeddings, and output-vocabulary IDs. | **LOCKED** |
...
| D-040 | HarMax context, memory, and branch pools use ordinary Euclidean geometry. Learned pairwise bilinear, Mahalanobis, and factored quadratic metrics are excluded from the contraction path. | **LOCKED** |
...
|---|---|---|---|
| 1 | Science-first 200M-word corpus; top 500k bigrams and top 500k trigrams | **IMPLEMENTED** | Modal run completed. The builder now resets at punctuation for future rebuilds. Existing rows produced solely across punctuation remain immutable but unreachable under the runtime word-boundary matcher. |
| 2 | Frozen Qwen donor Engrams | **IMPLEMENTED** | Separate bigram/trigram tables; full layer-8 and layer-24 BF16 vectors; smoke test and full extraction reported complete. |
...
| 3B | Layer-2 dictionary donor extractor | **CODED** | `modal_extract_definition_states.py`; resumable single-layer BF16 shard pipeline is code-complete; full Modal data run remains next. |
...
| 5 | JTD and sparse memory-fusion maps | **CODED** | Layer-8, layer-24, live-to-joint, and joint-to-live maps execute on CPU; `fit_joint_transfer_domain.py` fits them from same-content layer-2 anchor pairs. Full real-anchor fitting remains pending. |
...
Each physical block measures pool distance with ordinary Euclidean geometry
and an independent causal-displacement penalty:

\[
d_{iq}^2
=
\left\|h_i^{(r,k)}-z_{iq}\right\|_2^2
+
...
polarity. Let \(\widehat v_{\mathcal R,e,j}^{(r,k)}\) be the Euclidean unit
direction induced by the branch's bound source and conclusion anchors. Then:
...
The layer-2 definition vectors define the fixed reference geometry. Same-text
anchor pairs fit layer-8, layer-24, and live source maps with point alignment
and neighborhood preservation. Surface strings and IDs are excluded from the
numerical archive. The fitted checkpoint is loaded through
`DendritronLM.load_joint_transfer_checkpoint`.
...
> Run `modal run modal_prepare_definition_sources.py`. This CPU job downloads
> and fingerprints the locked OEWN 2025+, Wiktionary, and MeSH sources, compiles the
> complete curated single-word dictionary with every retained sense, reports
> its exact projected BF16 storage, and verifies 100% coverage of the completed
> Stage 2 vocabulary. It leaves both donor Engram banks untouched and uses CPU
> resources only.

Review `coverage_report.json`. Add reviewed source-grounded supplements for any
remaining Engram words, then freeze the source manifest at 100% Engram
coverage. Run:
...
The first command builds and validates the full CPU dictionary inventory. The
second begins resumable Qwen layer-2 extraction from that finalized inventory.

After the full dictionary bank passes validation:

1. Compile the CPU-only Qwen JTD index with:
   `modal run modal_build_joint_transfer_domain.py`.
2. Record Qwen's measured projection reduction, verify zero unresolved
   cryptographic collisions, and sample BOS/internal and punctuation-fusion
   round trips.
3. Train small recipient projections for active layer-8, layer-24, and layer-2
   rows.
4. Connect frozen JTD rows, Hash Engram, and the implemented LNGram module to
   the first recurrent core through their separate address namespaces.
5. Fit the 16-direction Universal/Shared-LoRA basis from successful task
   adapters and measure its explained variance in weight space.
