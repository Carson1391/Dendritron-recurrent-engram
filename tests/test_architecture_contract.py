...
class ArchitectureContractTests(unittest.TestCase):
    ...
    def test_context_module_uses_harmax_derivative_contract(self) -> None:
        ...
        self.assertNotIn("F.linear(displacement", source)
    ...
    def test_vocabulary_distance_is_chunked_euclidean(self) -> None:
        source = (PACKAGE_ROOT / "output_geometry.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("torch.cdist", source)
        self.assertIn("vocabulary_chunk_size", source)
        self.assertNotIn("state.unsqueeze(-2) - vocabulary", source)
